...
export default function JeePredictorPage() {
  ...
  useEffect(() => {
    fetch("/api/jee-predictor?metaOnly=true")
      .then((res) => res.json())
  ...
  const fetchData = useCallback(async (page = 1) => {
    ...
    const res = await fetch(`/api/jee-predictor?${params}`);
    const json = await res.json();
    setData(json.data);
    setPagination(json.pagination);
    setLoading(false);
    setSearched(true);
    // Reset inline filters on new search
  ...
  // Apply inline column filters to data
  ...
  // Unique values for inline filters from current data
  ...
  return (
    <div className="max-w-6xl">
      ...
      <div className="flex items-center gap-3 mb-1">
        ...
        <div>
          <h1 className="text-2xl font-bold text-green-900">JEE Advanced 2025 Predictor</h1>
          <p className="text-sm text-green-600/60">{meta.institutes.length} institutes · {meta.programs.length} programs · Rounds 1–6</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {mode === "college" && (
            <div className="sm:col-span-2 lg:col-span-3">
              <label className="block text-xs font-semibold text-green-800 mb-1">Your JEE Advanced Rank</label>
              <input type="number" value={rank} onChange={(e) => setRank(e.target.value)} placeholder="Enter your rank (e.g. 5000)"
          ...
          {mode === "rank" && (
            <div className="sm:col-span-2 lg:col-span-3">
              <SearchableMultiSelect
                ...
                placeholder="Search and select institutes..."
      ...
      {!searched && (
        <div className="text-center py-16 bg-white rounded-2xl border border-green-200/50">
          ...
          <p className="text-sm text-green-600/60 max-w-md mx-auto">
            {mode === "college"
              ? "We'll show you all IITs, NITs, IIITs & GFTIs where you can get admission based on JoSAA 2025 cutoff data."
