      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-neutral-200">
          <div className="px-5 py-4 border-b border-neutral-200">
            <h3 className="text-sm font-semibold text-neutral-900">Recent Payments</h3>
          </div>
          <div className="divide-y divide-neutral-100">
            {recentPayments.length === 0 && (
              <p className="text-sm text-neutral-400 p-5">No payments yet</p>
            )}
            {recentPayments.map((p) => (
              <div key={p.id} className="px-5 py-3 flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-neutral-900">{p.user.name}</p>
                  <p className="text-xs text-neutral-400">{p.type} • {new Date(p.createdAt).toLocaleDateString()}</p>
                </div>
                <span className="text-sm font-semibold text-neutral-900">₹{p.finalAmount}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-neutral-200">
          <div className="px-5 py-4 border-b border-neutral-200">
            <h3 className="text-sm font-semibold text-neutral-900">Pending Bookings</h3>
          </div>
          <div className="divide-y divide-neutral-100">
            {recentBookings.length === 0 && (
              <p className="text-sm text-neutral-400 p-5">No pending bookings</p>
            )}
