    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <Link href="/admin/assessments" className="p-1.5 hover:bg-neutral-100 rounded-lg transition">
          ...
          <div>
            ...
            <p className="text-xs text-neutral-500">{assessment.slug}</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          {message && (
      ...
      {activeTab === "details" && (
        <div className="bg-white rounded-xl border border-neutral-200 p-6 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-neutral-700 mb-1">Stream</label>
          ...
          <div>
            <label className="block text-sm font-medium text-neutral-700 mb-1">Overview</label>
            <textarea rows={3} value={details.overview} onChange={(e) => setDetails({ ...details, overview: e.target.value })}
              className="w-full px-3 py-2 border border-neutral-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-neutral-900 resize-none" />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-medium text-neutral-700 mb-1">Key Exams</label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-neutral-700 mb-1">Demand Rating (1-5)</label>
      ...
      {activeTab === "questions" && (
        <div className="space-y-4">
          {questions.map((q, i) => (
            <div key={q.id} className="bg-white rounded-xl border border-neutral-200 p-5">
              <p className="text-xs font-medium text-neutral-400 mb-3">Question {q.questionNumber}</p>
              <div className="space-y-3">
                <div>
                  <label className="block text-sm font-medium text-neutral-700 mb-1">Question Text</label>
          ...
          {questions.length === 0 && (
            <p className="text-sm text-neutral-500 text-center py-8">No questions yet. Questions are created via the seed script.</p>
          )}
        </div>
      )}
        ...
        <div className="space-y-6">
          {reports.map((r, i) => (
            <div key={r.id} className="bg-white rounded-xl border border-neutral-200 p-5">
              <p className="text-xs font-bold text-neutral-400 mb-4 uppercase tracking-wider">
                Tier: {r.tierLevel}
              </p>
              <div className="space-y-3">
                <div>
                ...
                {(["careerPaths", "topColleges", "entranceExamStrategy", "thirtyDayTest", "nextSteps", "mistakesToAvoid", "trendIntelligence"] as const).map((field) => (
                  <div key={field}>
                    ...
                    <textarea rows={3} value={r[field]} onChange={(e) => updateReport(i, field, e.target.value)}
                      className="w-full px-3 py-2 border border-neutral-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-neutral-900 resize-none font-mono text-xs" />
                  </div>
                ))}
              </div>
            </div>
          ...
          {reports.length === 0 && (
            <p className="text-sm text-neutral-500 text-center py-8">No reports yet. Reports are created via the seed script.</p>
          )}
        </div>
      )}
    </div>
